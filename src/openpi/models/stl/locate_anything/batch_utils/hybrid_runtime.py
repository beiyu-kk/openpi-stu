...
STRICT_ATTN = _env_flag("LA_FLASH_STRICT_ATTN", False)

def configure(model_path, *, device="cuda", dtype=torch.bfloat16, attn="sdpa", vision_attn="auto", strict_attn=False):
    """Configure the vendored single-model batch runtime before its first load."""
    global MODEL, DEV, DT, ATTN_MODE, REMOTE_ATTN_MODE, VISION_ATTN_MODE, STRICT_ATTN
    settings = (str(model_path), device, dtype, _normalize_attn_mode(attn), _normalize_vision_attn_mode(vision_attn), strict_attn)
    current = (MODEL, DEV, DT, ATTN_MODE, VISION_ATTN_MODE, STRICT_ATTN)
    if _model is not None and settings != current:
        raise RuntimeError("The batch runtime supports one model configuration per process")
    MODEL, DEV, DT, ATTN_MODE, VISION_ATTN_MODE, STRICT_ATTN = settings
    REMOTE_ATTN_MODE = "sdpa" if ATTN_MODE in {"la_flash", "magi", "eager"} else ATTN_MODE
...
def _remote_magi_diag(model=None):
    ...
    try:
        if model is not None:
            ...
            mod = importlib.import_module("..modeling_qwen2", package=__package__)
...
def load():
    """Lazy model load from local STL source plus release backends.
    ...
    if _model is None:
        ...
        components = load_components(MODEL, device=DEV, dtype=DT,
                                     text_attention=attn_impl, vision_attention=VISION_ATTN_MODE)
        _tok, _proc, _model = components.tokenizer, components.processor, components.model
