...
from .loader import load_components, resolve_model_path
...
class LocateAnythingWorker:
    ...
    def __init__(
        ...
        self.model_path = components.model_path
        self.loading_info = components.loading_info
    ...
    def _init_batch_runtime(
        ...
        """Initialize the locally vendored single-model batch runtime."""
        try:
            from .locate_anything.batch_utils import (
                generate_batch_hybrid,
                get_last_hybrid_stats,
                load,
            )
            from .locate_anything.batch_utils.hybrid_runtime import configure
        except ImportError as exc:
            raise ImportError(
                "The local LocateAnything batch runtime could not be imported."
            ) from exc
    ...
    def _crop_visual_prompt(
        ...
        if box_format == "normalized_1000":
            ...
            raise ValueError(
                "box_format must be 'normalized_1000', 'normalized_1', or 'pixel'"
            )
    ...
    def _replace_visual_prompt_text(
        ...
        if replace_text:
            if replace_text not in question:
                raise ValueError(
                    f"replace_text={replace_text!r} was not found in question"
                )
    ...
    def _build_messages(
        ...
        if visual_prompt_box is not None:
            visual_prompts.append(
                self._crop_visual_prompt(
                    image, visual_prompt_box, visual_prompt_box_format
                )
            )
        ...
        if visual_prompts:
            question = self._replace_visual_prompt_text(
                question, "<image-2>", replace_text
            )
        ...
        content.extend(
            {"type": "image", "image": prompt_image} for prompt_image in visual_prompts
        )
    ...
    def predict_batch(
        ...
        for item in requests:
            if isinstance(item, dict):
                ...
                if question is None:
                    raise ValueError(
                        "batch request dict must contain `question` or `prompt`"
                    )
                visual_prompt = item.get(
                    "visual_prompt", item.get("visual_prompt_image")
                )
                visual_prompt_box = item.get("visual_prompt_box")
                visual_prompt_box_format = item.get(
                ...
                if len(item) == 2:
                    ...
                    raise ValueError(
                        "batch tuple requests must be (image, question) or (image, question, visual_prompt)"
                    )

            has_visual_prompt = (
                has_visual_prompt
                or visual_prompt is not None
                or visual_prompt_box is not None
            )
            rows.append(
                {
                    "image": image,
        ...
        if generation_mode != "hybrid":
            raise ValueError(
                "batch runtime currently supports generation_mode='hybrid'"
            )
    ...
    def detect_visual_prompt(
        ...
        **kwargs,
        ...
        if visual_prompt is None and visual_prompt_box is None:
            raise ValueError(
                "detect_visual_prompt requires visual_prompt or visual_prompt_box"
            )
    ...
    def detect_batch(
        self, requests: list[tuple[Image.Image, Union[list[str], str]]], **kwargs
    ) -> list[dict]:
        ...
        for image, categories in requests:
            cats = (
                categories if isinstance(categories, str) else "</c>".join(categories)
            )
    ...
    parse_boxes = staticmethod(parse_boxes)
    parse_points = staticmethod(parse_points)


class SceneTextLocator:
    """In-memory image API using the same worker as the single-image CLI."""
    ...
    def __init__(self, config):
        ...
        self.worker = LocateAnythingWorker(
            ...
            text_attention=config.text_attention,
            vision_attn=config.vision_attention,
            revision=config.revision,
            local_files_only=config.local_files_only,
        )

    def locate(self, image, *, task="ground-text", query=None, categories=None):
        ...
        if isinstance(image, Image.Image):
            ...
            if image.ndim != 3 or image.shape[-1] != 3 or image.dtype != np.uint8:
                raise ValueError("STL images must be HWC RGB uint8 arrays")
        ...
        return STLResult(
            ...
            points=parse_points(answer, *image.size),
